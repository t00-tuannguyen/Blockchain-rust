...
impl Wallet {
    ...
    pub fn generate_wallet(accounts: &mut Account) {
        let mut csprng = OsRng {};
        let keypair = Keypair::generate(&mut csprng);
        let pub_key = hex::encode(keypair.public.to_bytes());
        println!("Your Public Key {}", pub_key);
        println!("Your Key Pair {:?}", hex::encode(keypair.to_bytes()));
        let address = pub_key.clone();
        accounts.accounts.push(address.clone());
        accounts.initialize(&address);
        // self.get_balance(blockchain);
    }
